...
### Clade:applyVector\(Vec\)

Applies the Vector by copying the subtables to avoid sharing structure\.

An anticipated pattern with a Vector involves some massaging the phyle names
before coalescence\.
...
Some grammars are more useful if rules can be collapsed into one synonym,
