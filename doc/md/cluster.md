...
raises the return value into an assertion\. \#Todo \#NYI
...
a Panda is a sort of Raccoon, we might change our mind and say it is in fact
a Bear\.  In stark contract with biology, it is our saying so which makes it
the case\.  If our Roguelike calls for the Quacking Panda Duck\-Bear, so be it\.
...
extended via `cluster.genus`\.  All subsequent extensions are a genus, that
is, genera\.  Both order and genus are genres, and we speak of genres as being
a basis of, generic to, and specific from, one another\.
...
#### \#Next Storing the Contract

The obvious place is just `__meta.contract`\.  I have no good argument against
the obvious\.

The table the users passes for the Contract, must remain the Contract, so
specialization will need to apply the generic Contract or Contracts to the
supplied table\.


##### Cluster:contract module

Useful code, probably in the wrong namespace\.

Hence the cluster house style of no localizations until further notice\.
...
A contract is applied with [applycontract](https://gitlab.com/special-circumstance/cluster/-/blob/trunk/doc/md/applycontract.md), passing `nil` as
...
- [0/3]  Contract:  [#Todo]
  ...
  - [ ]  Refactor `order` with contract module

  - [ ]  Refactor `genus` ibid\.

  - [ ]  Redesign the cluster library interface in light of a year's
      developments\.
...
metaobject protocol, whereas class is used extensively in the sense of
...
###### A Third, More Complex Thing

The `boundindex` assumes that we need to close over state, but have a single
metatable\.  It's at least as common for us to want to close over parameters
provided during construction, and this requires creating a distinctive
endowment for each subject\.

We want that anyway, as simple as "put a metatable on it" might be, it's
essential to reviving database and serialized subjects, where we can make the
endowments into fenv globals and call them on the literal table constructors\.


##### Extending and Replacing the Library

The seed, by convention, has the tape as its index\.

There are occasions when we want to provide additional functions, canonically
the building blocks of more complex methods where extensions might need to
use them in lieu of a simple super call\.
