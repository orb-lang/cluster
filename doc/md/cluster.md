...
languages tend to do with classes, Lua does with hashtables and a clever
...
```lua
local Seed, Cassette, Meta = setmetatable({}, {}), {}, {__meta = {}}
...
Although, of course, not created out in the open like that\.
...
So I'm feeling comfortable giving this a go\.
...
Two important points about the builder: It should expect a seed as the first
argument, and it **must not** assign a metatable\.

 \#Todo we should arguably enforce this with a fenv, because Mike Pall has said
  that the JIT will hate you if you ever reassign the metatable of a given
  table\.
...
Cluster takes care of making the builder into a constructor by composing an
