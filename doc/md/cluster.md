...
identical to the Cassette\.  Cluster will handle function indices as well in
...
The Cassette with `boundindex` lives at  `__meta.keys`, where the contract is
...
be of assistance\.  It is unclear what serves as the Cassette under these
circumstances\.

Also an `instruments_may_index` flag, meaning that despite the scary function,
the keys which end in `__meta.keys` may be applied to that function to get
a result\.  It's a promise of no side\-effects\.

This isn't a security thing, it's a politeness thing; Lua is nasal demons all
the way down\.


##### seedmeta

Normally, the Cassette is assigned to the Seed metatable as the index, as
...
values from the cassette during building, whether that's a good idea or notand it can be, as a way to design a more\-generic builder which dispatches on
qualities
